...
#2
sdna="ATGCATCATGATGCTGATCG"
n=2 # more than this number found

for k in list(range(2,6,1)): # # kmer size from 2 to 5
  kmer=[]
  for i in list(range(0,len(sdna)-int(k)+1,1)):
...
for i  in list(range(0,3,1)):
  for m in list(range(0,3,1)):
  count=0
  for k in list(range(0,8,1)):
    if dnas[i][k] == dnas[m][k]:
      count+=1
  print("seq",i+1,'and seq',m,', the percentage of identical positions is',100*count//9,'%') 
